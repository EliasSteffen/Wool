...
const DEFAULT_WATER_RESISTANCE: float = 0.95
const DEFAULT_UPWIND_FORCE: float = -1200.0
...
func _ready() -> void:
	settings = {
		...
		"Water": {
			...
			"buoyancy_force": { "value": DEFAULT_WATER_BUOYANCY_FORCE, "min": -500.0, "max": 0.0, "step": 10.0 },
			"water_resistance": { "value": DEFAULT_WATER_RESISTANCE, "min": 0.8, "max": 1.0, "step": 0.001 }
		},
		"Upwind": {
			"upwind_force": { "value": DEFAULT_UPWIND_FORCE, "min": -5000.0, "max": -500.0, "step": 100.0 }
		}
	}
